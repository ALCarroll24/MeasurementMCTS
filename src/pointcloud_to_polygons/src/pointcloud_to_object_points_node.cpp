...
private:
    void pointCloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
    {
        ...
        for (const auto &indices : cluster_indices)
        {
            ...
            // Create a Marker to represent the points
            visualization_msgs::msg::Marker marker;
            marker.header.frame_id = msg->header.frame_id;
            marker.header.stamp = msg->header.stamp;
            marker.ns = "points";
            marker.id = cluster_id++;
            marker.type = visualization_msgs::msg::Marker::POINTS;  // Change to POINTS type
            marker.action = visualization_msgs::msg::Marker::ADD;

            // Set the scale for the points (x and y set to the point size)
            marker.scale.x = 0.1;  // Point width
            marker.scale.y = 0.1;  // Point height

            // Set color for the marker
            marker.color.r = 1.0;  // Example: Red color
            marker.color.g = 0.0;
            marker.color.b = 0.0;
            ...
            for (const auto &point : cluster->points)
            {
                ...
                p.x = point.x;
                p.y = point.y;
                p.z = max_height;  // Use max height for all points in the cluster
